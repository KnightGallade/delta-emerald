...
u32 DetermineFollowerNPCState(struct ObjectEvent *follower, u32 state, u32 direction)
{
    ...
    if (IsPlayerForcedOntoSameTile(newPlayerMB, playerMoveDirection)
     && !(TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_BIKE) && gSaveBlock2Ptr->playerBike == ACRO_BIKE && playerMoveDirection == DIR_NORTH && newPlayerMB == MB_MUDDY_SLOPE && GetPlayerSpeed() >= PLAYER_SPEED_FAST))
    {
