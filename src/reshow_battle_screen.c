...
static bool8 LoadBattlerSpriteGfx(u32 battler)
{
    if (battler < gBattlersCount)
    {
        if (!IsOnPlayerSide(battler))
        {
            ...
            DecompressTrainerBackPic(GetCurrentAvatarGender(), battler);
...
void CreateBattlerSprite(u32 battler)
{
    if (battler < gBattlersCount)
    {
        ...
        {
            SetMultiuseSpriteTemplateToTrainerBack(GetCurrentAvatarGender(), GetBattlerPosition(B_POSITION_PLAYER_LEFT));
            gBattlerSpriteIds[battler] = CreateSprite(&gMultiuseSpriteTemplate, 0x50,
                                                (8 - gTrainerBacksprites[GetCurrentAvatarGender()].coordinates.size) * 4 + 80,
